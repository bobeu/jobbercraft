...
const index = () => {
  return (
    <section className='w-full bg-cyan-900 text-cyan-300 py-4 ' id='#jobbers'>
      <div className='flex justify-between px-4 items-center text-2xl w-full h-[42px]'>
        ...
        <UserInputBase 
          ...
          className='bg-cyan-200 hover:bg-white'
        />
      </div>
      <div className="w-full h-full container grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 p-4 ">
        {
          JOBBERS.map((jobber, i) => (
            <div key={i} className="w-full p-14 rounded-md bg-cyan-900 border border-cyan-700 text-cyan-300 hover:bg-cyan-800 cursor-pointer shadow-lg shadow-gray-600">
              <div className='w-full flex justify-between items-center p-4 rounded gap-2 text-white shadow-sm shadow-yellow-500'>
